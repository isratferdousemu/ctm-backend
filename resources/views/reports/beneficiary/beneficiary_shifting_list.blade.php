...
<table class="border-table">
    ...
    @foreach($beneficiaries as $index => $beneficiary)
        <tr>
            ...
            <td>{{($beneficiary?->city_corporation_name_bn ?: ($beneficiary?->district_pourashava_name_bn ?: $beneficiary->upazila_name_bn))}}</td>
            <td>{{($beneficiary->thana_bn ?: ($beneficiary->union_bn ?: $beneficiary->pourashava_name_bn))}}</td>
            <td>{{$beneficiary?->ward_bn}}</td>
            <td>{{$beneficiary?->from_program_name_bn}}</td>
            <td>{{$beneficiary?->to_program_name_bn}}</td>
            <td>{{$beneficiary->shifting_cause}}</td>
            <td>{{\App\Facades\BengaliUtil::bn_date(\Illuminate\Support\Carbon::parse($beneficiary->activation_date)->format('d/m/Y'))}}</td>
        </tr>
    @endforeach
    </tbody>
</table>
